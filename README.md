# trabalho-portas
# Entradas 
São aonde os numeros são emitidos, e é informado se havera a necessidade do uso do carry naquela operação
# Saidas
São aonde geram os resultados, enviando já para operação final, ou emitindo o carry e avançando uma casa decimal.
# Explicação
Assim que informados valores, o half-adder e o full-adder ficam encarregados de gerar a informação se há necessidade do uso do carry na operação, assim se o carry for usado algumas vezes, ele chegaram no limite permitido pelo circuito
